...
// Direct Message Send API - Round-robin fast sending
app.post('/api/send-dm', async (req, res) => {
    try {
        ...
        const minDelay = Math.max(parseInt(delayMs) || 100, 100);
...
`;
        ...
        const totalMessages = tokens.length * channelIds.length * messageCount;
        console.log(`[send-dm] START: ${tokens.length}tok x ${channelIds.length}ch x ${messageCount}msg = ${totalMessages}`);

        const results = { success: 0, failed: 0, total: 0, channels: {} };
        for (const cid of channelIds) {
            results.channels[cid] = { success: 0, failed: 0, remaining: messageCount };
        }

        for (const token of tokens) {
            const headers = getDiscordHeaders(token);
            const channelState = new Map();
            channelIds.forEach(cid => channelState.set(cid, { sent: 0, waitUntil: 0 }));
            
            let totalSent = 0;
            const target = channelIds.length * messageCount;
            ...
            while (totalSent < target) {
                ...
                for (const channelId of channelIds) {
                    ...
                    if (state.sent >= messageCount) continue;
                    ...
                    try {
                        ...
                        results.total++;
                        sentThisRound = true;
                        console.log(`[send-dm] OK ${channelId.slice(-4)} [${state.sent}/${messageCount}] total:${totalSent}/${target}`);
                        
                    } catch (err) {
                        if (err.response?.status === 429) {
                            const wait = (parseFloat(err.response?.data?.retry_after || 1) * 1000) + 200;
                ...
                if (!sentThisRound) {
                    const minWait = Math.min(...[...channelState.values()].map(s => s.waitUntil)) - Date.now();
                    if (minWait > 0) {
                        await delay(Math.min(minWait + 100, 2000));
                    } else {
                        await delay(500);
                    }
                }
        ...
        console.log(`[send-dm] DONE: ${results.success}/${results.total}`);
